            ...
            <section class="content container-fluid">
                ...
                <form  method="post" enctype="multipart/form-data" action="{{route('slider.update',$slider->id)}}" >
                    ...
                    @foreach($urun as $Urun)
                      <option value="{{$Urun->id}}" selected>{{$Urun->urun_adi}}</option>
                      @endforeach
                    ...
                    <select id="il" name="il">
                     <option value="">Select an il</option>
                        @foreach ($il as $iller)
                           <option value="{{ $iller->il_adi }}" selected>{{ $iller->il_adi }}</option>
                        @endforeach
                    </select>
                     ...
                     <select id="ilce" name="ilce">
                      <option value="">Select an ilce</option>
                        @foreach ($ilceler as $ilce)
                           <option value="{{ $ilce->ilce_adi }}" selected>{{ $ilce->ilce_adi }}</option>
                        @endforeach
                      </select>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <input type="number" name="EkimAlani" class="form-control" id="formGroupExampleInput2" placeholder="EkimAlani girin" value="{{$slider->EkimAlani}}">
                    </div>
                    ...
                    @foreach($cikis as $Cikis)
                      <option value="{{$Cikis->Cikis_Durumu}}" selected>{{$Cikis->Cikis_Durumu}}</option>
                    @endforeach
                    </select>
                    
                    <label for="gelisme-bakim-durumu">Gelişme ve Bakım Durumu</label>
                    <select id="gelisme_bakim_durumu" name="gelisme_bakim_durumu">
                    <option value="">Gelisme durumunu secin</option>
                    @foreach($gelisme as $Gelisme)
                       <option value="{{$Gelisme->Gelisme}}" selected>{{$Gelisme->Gelisme}}</option>
                       @endforeach
                    ...
                    @foreach($Ay as $ay)
                       <option value="{{$ay->Ay_adi}}" selected>{{$ay->Ay_adi}}</option>
                       @endforeach
                     ...
                     <textarea id="Gozlem-Durumu" name="Gozlem_Durumu" rows="5" cols="50">{{$slider->GozlemDurumu}}</textarea>
